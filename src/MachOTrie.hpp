...
struct Node {
  Node(const char *s)
      : fCummulativeString(s), fAddress(0), fFlags(0), fOther(0),
        fImportedName(NULL), fOrdered(false), fHaveExportInfo(false),
        fTrieOffset(0) {}
  ~Node() {}
  ...
  void addSymbol(const char *fullStr, uint64_t address, uint64_t flags,
                 uint64_t other, const char *importName) {
    const char *partialStr = &fullStr[strlen(fCummulativeString)];
    for (std::vector<Edge>::iterator it = fChildren.begin();
         it != fChildren.end(); ++it) {
      Edge &e = *it;
    ...
    // no commonality with any existing child, make a new edge that is this
    // whole string
    ...
    if ((flags & EXPORT_SYMBOL_FLAGS_REEXPORT) && (importName != NULL) &&
        (strcmp(fullStr, importName) != 0))
  ...
  bool updateOffset(uint32_t &offset) {
    ...
    if (fHaveExportInfo) {
      ...
      // do have export info, overall node size so far is uleb128 of
    ...
    for (std::vector<Edge>::iterator it = fChildren.begin();
      ...
      Edge &e = *it;
      nodeSize +=
          strlen(e.fSubString) + 1 + uleb128_size(e.fChild->fTrieOffset);
    }
    ...
    if (fHaveExportInfo) {
      if (fFlags & EXPORT_SYMBOL_FLAGS_REEXPORT) {
        if (fImportedName != NULL) {
          ...
          uint32_t nodeSize =
              (uint32_t)(uleb128_size(fFlags) + uleb128_size(fOther) +
                         strlen(fImportedName) + 1);
          out.push_back(nodeSize);
          append_uleb128(fFlags, out);
          append_uleb128(fOther, out);
          append_string(fImportedName, out);
        } else {
          // nodes with re-export info: size, flags, ordinal,
          // empty-string
...
inline void makeTrie(const std::vector<Entry> &entries,
  ...
  for (std::vector<Entry>::const_iterator it = entries.begin();
       it != entries.end(); ++it) {
    start.addSymbol(it->name, it->address, it->flags, it->other,
                    it->importName);
  }
  ...
  do {
    uint32_t offset = 0;
    more = false;
    for (std::vector<Node *>::iterator it = orderedNodes.begin();
...
struct EntryWithOffset {
  ...
  bool operator<(const EntryWithOffset &other) const {
    return (nodeOffset < other.nodeOffset);
  }
};

static inline void processExportNode(const uint8_t *const start,
                                     const uint8_t *p, const uint8_t *const end,
  ...
  for (uint8_t i = 0; i < childrenCount; ++i) {
    ...
    cummulativeString[curStrOffset + edgeStrLen] = *s++;
    uint32_t childNodeOffset = (uint32_t)read_uleb128(s, end);
    if (childNodeOffset == 0)
      throw "malformed trie, childNodeOffset==0";
    processExportNode(start, start + childNodeOffset, end, cummulativeString,
                      curStrOffset + edgeStrLen, output);
  }
}

inline void parseTrie(const uint8_t *start, const uint8_t *end,
  ...
  for (std::vector<EntryWithOffset>::iterator it = entries.begin();
       it != entries.end(); ++it)
