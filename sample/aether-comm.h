...
#else
static const char *llvm_name = "LLVM.dll";
...
static bool load_libraries(std::string_view script_file) {
  ...
  auto libllvm = proj_dir / "build-llvm" / "install" / "lib" / llvm_name;
  if (!icpp::load_library(libllvm.string())) {
    std::print("Failed to load {}", libllvm.string());
    return false;
  }
  for (std::string_view type : {"build-Debug", "build-Release"}) {
    auto libaether = proj_dir / type / aether_name;
    if (fs::exists(libaether)) {
      if (icpp::load_library(libaether.string()))
        break;
      std::print("Failed to load {}", libaether.string());
      return false;
    }
  }
  return true;
